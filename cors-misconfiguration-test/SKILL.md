...
description: Testowanie konfiguracji CORS pod kątem podatności (overly permissive origins, credentials bypass, wildcard origins). Triggery — "sprawdź CORS", "audytuj cross-origin", "czy API jest podatne na CORS", "test bezpieczeństwa endpointów". Adapted from mukul975/Anthropic-Cybersecurity-Skills (Apache-2.0).
